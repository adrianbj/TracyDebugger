...
(function(){
				...
				if (this.rel === 'close') {
					...
					if (e.shiftKey) {
						...
						setPosition(panel.elem, {
							left: getPosition(panel.elem).left - Math.round(Math.random() * 100) - 20,
							top: getPosition(panel.elem).top - (Math.round(Math.random() * 100) + 20) * (_this.isAtTop() ? -1 : 1)
						});
					}
				}
				e.preventDefault();
	...
	Debug.loadAjax = function(content, dumps) {
		...
		if (!Debug.bar.isAtTop()) {
			setPosition(Debug.bar.elem, {right: pos.right, bottom: pos.bottom});
		}
