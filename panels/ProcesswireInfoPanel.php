...
class ProcesswireInfoPanel extends BasePanel {
    ...
    public function getTab() {
        ...
        Debugger::timer('processwireInfo');
...
HTML;
        ...
        if(in_array('processWireWebsiteSearch', $panelSections)) {
            ...
            <script{$nonceAttr}>
                ...
                var idLookupForm = document.getElementById('tracyIdLookupForm');
                if (idLookupForm) {
                    idLookupForm.addEventListener('submit', function(e) { e.preventDefault(); });
                }
                var searchPwForm = document.getElementById('tracySearchPwForm');
                if (searchPwForm) {
                    searchPwForm.addEventListener('submit', function(e) { e.preventDefault(); searchPw(this); });
                }
            </script>
HTML;
        }
...
HTML;
            ...
            if(function_exists('gd_info')) {
                ...
                $versionsDetails['GD Settings']['items']['AVIF'] = (isset($gd['AVIF Support']) ? $gd['AVIF Support'] : false);
            }
            ...
            if(class_exists('\Imagick')) {
                ...
                $versionsDetails['iMagick Settings']['items']['AVIF'] = (in_array('AVIF', $im) ? 1 : false);
            }
        ...
        if(in_array('adminLinks', $panelSections)) {
            ...
            <ul class="pw-info-links">
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="'.$this->wire('config')->urls->admin.'" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                if($this->wire('user')->isLoggedIn()) {
                    ...
                    $out .= '
                    <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                        <a data-tracy-close="ProcesswireInfo" href="'.TracyDebugger::inputUrl(true) . (strpos(TracyDebugger::inputUrl(true), '?') !== false ? '&' : '?') . 'tracyLogout=1"'.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    $out .= '
                    <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                        <a data-tracy-close="ProcesswireInfo" href="'.$this->wire('config')->urls->admin . '?tracyLogin=1"'.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="'.TracyDebugger::inputUrl(true) . (strpos(TracyDebugger::inputUrl(true), '?') !== false ? '&' : '?') . 'tracyClearSession=1"'.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="'.TracyDebugger::inputUrl(true) . (strpos(TracyDebugger::inputUrl(true), '?') !== false ? '&' : '?') . 'tracyModulesRefresh=1"'.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="'.$this->wire('config')->urls->admin.'module/edit?name=TracyDebugger" '.$this->newTab.' '.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>'
                        .$this->tracyIcon.
                        ($withLabels ? '&nbsp;'.$linkTitle.'</a>' : '</a>&nbsp;') .
                '</li>
            </ul>';
...
HTML;
            ...
            foreach(TracyDebugger::getDataValue('customPWInfoPanelLinks') as $path) {
                ...
                if(strpos($cp->url, '/upgrades/') !== false) {
                    $cpUrl = str_replace('/upgrades/', '/upgrades/refresh', $cp->url);
                }
                else {
                    $cpUrl = $cp->url;
                }
                ...
                $out .=
                '<li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" '.$this->newTab.' href="'.$cpUrl.'"'. (!$withLabels ? ' title="'.htmlspecialchars($cp->title ?? '', ENT_QUOTES, 'UTF-8').'"' : '') . '>
        ...
        if(in_array('documentationLinks', $panelSections)) {
            ...
            $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                <a data-tracy-close="ProcesswireInfo" href="https://github.com/processwire/processwire" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="https://processwire.com/talk/" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="https://processwire.com/docs/" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="https://processwire.com/api/ref/" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="http://cheatsheet.processwire.com/" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>
                ...
                $out .= '
                <li ' . ($withLabels ? ' class="with-labels"' : '') . '>
                    <a data-tracy-close="ProcesswireInfo" href="https://adrianbj.github.io/TracyDebugger/" '.$this->newTab.(!$withLabels ? ' title="'.$linkTitle.'"' : '').'>'
                        .$this->tracyIcon.
                        ($withLabels ? '&nbsp;'.$linkTitle.'</a>' : '</a>&nbsp;') .
                '</li>
            </ul>';
            ...
            if(in_array('gotoId', $panelSections)) {
                $out .= '
                <form id="tracyIdLookupForm" style="border-top: 1px solid #CCCCCC; margin:10px 0 0 0 ; padding: 10px 0 0 0;">
                    <input id="pageId" name="pageId" placeholder="Goto Page ID" type="text" autocomplete="off" />
                    <a data-tracy-close="ProcesswireInfo" href="#" class="tracyLinkBtn" id="idGoToView" />View</a>
                    <a data-tracy-close="ProcesswireInfo" href="#" class="tracyLinkBtn" id="idGoToEdit" />Edit</a>
                    <a data-tracy-close="ProcesswireInfo" href="#" class="tracyLinkBtn" id="idGoToOpen" />Open</a>
            ...
            if(in_array('processWireWebsiteSearch', $panelSections)) {
                $out .= '
                <form id="tracySearchPwForm" style="border-top: 1px solid #CCCCCC; margin:10px 0 0 0 ; padding: 10px 0 0 0;">
