...
class GitInfoPanel extends BasePanel
{
    ...
    function getTab() {
        ...
        return $this->buildTab('Git Info: '.$this->branchName, $this->branchName);
    }
    ...
    function getPanel() {
        if($this->isUnderVersionControl()) {
            $out = $this->buildPanelHeader('Git Info');
            $warning = '';
            $cntTable = '';
            ...
            $out .= $this->openPanel('tracy-InfoPanel') . '<table><tbody>' .
                $cntTable .
                '</tbody></table>';
