 ...
 * 
 * To make your panel visible you have to add it to your public static $allPanels array
 *
...
class WebHooksPanel extends BasePanel {
    ...
    private $label = 'WebHooks Panel';

    // the svg icon shown in the bar and in the panel header
    private $icon = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 28.477 28.477" style="enable-background:new 0 0 28.477 28.477;" xml:space="preserve">
      <g>
        <path d="M14.286,9.502c-1.557,0-2.824,1.263-2.824,2.823c0,0.799,0.338,1.519,0.872,2.032v14.12h4.001v-14.22   c0.479-0.505,0.775-1.183,0.775-1.932C17.11,10.766,15.847,9.502,14.286,9.502z"></path>
    ...
    public function getDumps() {
        $dir = $this->wire->config->paths->cache . "TracyDebugger/WebHookLogger";
        $tracy = $this->wire->modules->get('TracyDebugger');

        ob_start();
        echo "<div class='tracy-DumpPanel'>";
        ...
        foreach($files as $key=>$file) {
            ...
            $data = $tracy->getWebHookData($id);
            $time = date("d.m.Y H:i:s", $data->time); // todo: international?
            TD::dump($data, "$time, Request $id");
        }
