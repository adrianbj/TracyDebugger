...
Object.assign(tracyConsole, {
            ...
            tracyIncludeCode: function(when) {
                when = when.value;
                const params = {when: when, pid: tracyConsole.pid};
            ...
            cleanupRunFiles: function(runId) {
                ...
                xhr.open("POST", tracyConsole.currentUrl, true);
            ...
            pollForStream: function(runId, tabId, offset, quietCount, gen) {
                ...
                xmlhttp.open("POST", tracyConsole.currentUrl, true);
            ...
            callPhp: function(code, codeReturn = true) {
                ...
                xmlhttp.send("tracyConsole=1&runId=" + encodeURIComponent(runId) + "&csrfToken=" + encodeURIComponent(this.csrfToken) + "&codeReturn=" + encodeURIComponent(codeReturn) + "&allowBluescreen=" + encodeURIComponent(allowBluescreen) +
                    ...
                    "&accessTemplateVars=" + encodeURIComponent(accessTemplateVars) + "&pid=" + encodeURIComponent(tracyConsole.pid) +
                    "&fid=" + encodeURIComponent(tracyConsole.fid) + "&tid=" + encodeURIComponent(tracyConsole.tid) + "&mid=" + encodeURIComponent(tracyConsole.mid) +
        ...
        tracyJSLoader.load(tracyConsole.tracyModuleUrl + "scripts/ace-editor/ace.js", function() {
            if (typeof ace !== "undefined") {
                ...
                tracyConsole.tce.setShowInvisibles(tracyConsole.codeShowInvisibles);
                ...
                const initializeTabs = async function() {
                    ...
                    try {
                        ...
                        const serverSnippets = tracyConsole.snippets;
                ...
                ace.config.loadModule('ace/ext/language_tools', function() {
                    tracyConsole.tce.setOptions({
                        ...
                        tabSize: tracyConsole.codeTabSize,
                        useSoftTabs: tracyConsole.codeUseSoftTabs,
                        minLines: 5
                    });

                    if (tracyConsole.pwAutocomplete.length > 0 || tracyConsole.pwAutocompleteUrl !== '') {
                    ...
                    if (tracyConsole.pwAutocompleteUrl !== '' && !tracyConsole.pwAutocompleteLoaded) {
                        tracyConsole.pwAutocompleteLoaded = true;
                        fetch(tracyConsole.pwAutocompleteUrl, { credentials: 'same-origin' })
                            .then(function(response) {
                                return response.ok ? response.json() : [];
                            })
                            .then(function(apiCompletions) {
                                if (Array.isArray(apiCompletions) && apiCompletions.length > 0) {
                                    tracyConsole.pwAutocomplete = tracyConsole.pwAutocomplete.concat(apiCompletions);
                                }
                            })
                            .catch(function() {
                                tracyConsole.pwAutocompleteLoaded = false;
                            });
                    }
                    ...
                    toggleFullscreenButton.innerHTML = '<span class="fullscreenToggleButton" title="Toggle fullscreen">' + tracyConsole.maximizeSvg + '</span>';
