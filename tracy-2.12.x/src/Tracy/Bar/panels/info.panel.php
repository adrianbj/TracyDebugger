...
$countClasses = fn(array $list): int => count(array_filter($list, fn(string $name): bool => (new \ReflectionClass($name))->isUserDefined()));
...
// TracyDebugger patch: opcache_get_status() defaults to $include_scripts=true, which materialises
// per-script stats for every entry in the opcode cache - cache-wide, so every vhost sharing the PHP
// pool inflates it. Measured ~157ms of a ~250ms request with 1365 cached scripts, over half the
// debug bar. All this row needs is "was each of *this* request's files cached", which
// opcache_is_script_cached() answers with one hash lookup per file (~0.06ms total, same figure).
// The 'OPcache' row below is unchanged. Upstream PR: https://github.com/nette/tracy/pull/623
...
$cachedFiles = $opcache && function_exists('opcache_is_script_cached')
	? array_filter(get_included_files(), function ($file) { return @opcache_is_script_cached($file); })
	: [];
$jit = $opcache['jit'] ?? null;

$info = [
	'Execution time' => number_format($this->time * 1000, 1, '.', "\u{202f}") . "\u{202f}ms",
	'CPU usage user + system' => isset($userUsage, $systemUsage) ? (int) $userUsage . "\u{202f}% + " . (int) $systemUsage . "\u{202f}%" : null,
	'Peak of allocated memory' => number_format(memory_get_peak_usage() / 1_000_000, 2, '.', "\u{202f}") . "\u{202f}MB",
...
if (class_exists(ClassLoader::class, autoload: false)) {
	$baseDir = (function () {
		$baseDir = null;
		@include dirname((string) (new \ReflectionClass(ClassLoader::class))->getFileName()) . '/autoload_psr4.php'; // @ may not exist
		return $baseDir;
	})();
	...
	foreach ([&$packages, &$devPackages] as &$items) {
		...
		usort($items, fn($a, $b): int => $a->name <=> $b->name);
	}
}
